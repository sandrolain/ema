...
declare global {
  interface HTMLElementTagNameMap {
    'ema-numbers': EmaNumbersElement
  }
}

type BigLetter = {letter: string; uid: number};

@customElement('ema-numbers')
export class EmaNumbersElement extends LitElement {
  static styles = css`
    :host {
      display: block;
      width: 100%;
  ...
  render() {
    return html`
      <div id="letters">${numbers.map((n) => html`<button @click=${() => this.playLetter(n)}>${n}</button>`)}</div>
      <div id="big-letters">
